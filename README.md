...
なぜなぜ君の品質を維持するため、以下の動作を確認してください：

### 質問候補ボタンの品質
- **短縮表示の品質**: ボタンの表示（short）が18〜22文字以内で意味が通じること。補足（括弧内など）が削除され、主節が優先されていること
- **候補の多様性**: 3つのボタンが重複せず、異なる観点の質問になっていること
- **長い候補への対応**: 80文字を超える長い質問候補でもUIが崩れず、安全に表示されること

### 更新ボタンの文脈対応
- **初回（履歴なし）**: 更新ボタン（↻）をタップすると、ランダムな候補が表示される
- **会話中（履歴あり）**: 更新ボタンをタップすると、直近の会話内容に関連した候補が表示される（Workers APIから取得）
- **失敗時のフォールバック**: Workers APIが失敗した場合でも、ランダム候補にフォールバックして動作が継続すること

### 送信時の候補更新
- **質問送信後**: 回答が表示されると同時に、3つの質問ボタンが新しい候補に自動更新されること
- **文脈の継続性**: 更新された候補が、直前の回答に関連した「さら問い」になっていること

### 品質ガードの動作
- **重複検出**: Workersから重複した候補が返ってきた場合、フォールバック候補が表示されること（コンソールに警告が表示される）
- **空・無効な候補**: 候補が空や無効な場合でも、常に3つのボタンが表示されること

これらの確認により、ユーザーが迷わず、スムーズに会話を深掘りできる体験が保証されます。
